...
export async function goTo({ playwrightPage, puppeteerPage, timeout, url }: Params) {
  ...
  await Promise.race([
    playwrightPage
      ...
      .catch(() => {})
      .then(() => {
        // `/api/v1/admin/me` API はエラーを返す可能性があるので無視する
        const ignoredUrls = watcher.getRequestedUrls().filter((url) => url.includes('/api/v1/admin/me'));
    ...
    setTimeout(timeout ?? 60 * 1000).then(() => Promise.reject(new Error('ページの読み込みがタイムアウトしました'))),
  ]);
